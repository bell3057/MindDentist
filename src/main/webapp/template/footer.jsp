<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
    
<link rel="stylesheet" type="text/css" src="../css/template.css">

<div id="footer_wrap">
	<div style="margin: auto; width: 960px; height: 100%">
		<div style="width: 236px; float: left;">
			<img id="footer_logo_img" src="../img/logo.png">
			<span id="copyright">Copyright © Mind Dental All rights reserved.</span>
		</div>
		<div id="footer_company">안산시 단원구 고잔로 72 센트럴타워 4층 
		개인정보관리책임자 : 양재민 l minddental@naver.com 
		대표 : 양재민 l 사업자등록번호 570-18-00692 
		Tel : 031-401-9375 l Fax : 031-411-9375
		</div>
		<div style="width: 132px; height: 60px; float:left; margin-top: 66px;">
		<img class="logo_png insta_logo" src="../img/Main/insta.png" style="margin-right:6px;">
		<img class="logo_png blog_logo" src="../img/Main/blog.png">
		</div>
	</div>
</div>
<div id="footer_mobile">
	<div class="plate_arrange">
		<div class="fd1">
			<div class="fd1d1">
				<img src="../img/logo.png">
				<div>Copyright © Mind Dental All rights reserved.</div>
			</div>
			<div class="fd1d2">
				<img class="insta_logo" src="../img/Main/insta.png">
				<img class="blog_logo" src="../img/Main/blog.png">
			</div>
		</div>
		<div class="fd2">안산시 단원구 고잔로 72 센트럴타워 4층
		개인정보관리책임자 : 양재민 l minddental@naver.com
		대표 : 양재민 l 사업자등록번호 570-18-00692
		Tel : 031-401-9375 l Fax : 031-411-9375</div>
	</div>
</div>
<script type="text/javascript" src="http://code.jquery.com/jquery-3.4.1.min.js"></script>
<script src="../js/template.js"></script>