<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
    
<link rel="stylesheet" type="text/css" href="../css/inform.css"/>

<div id="inform_wrap-4">
	<div class="top-bar top-bar4 topBarPC">
		<div class="pSysd1">
			<div id="pSystem_menu-1" style="float: left;">
				<div id="pSystem_menu_text-1" class="pSystem-text-1" style="color: #C8500F; font-weight: 800;">무통마취 시스템</div>
				<div id="pSystem_menu_selected-1" class="pSystem_selected" style="width: 165px; height: 4px; background-color: #B4AAA0;"></div>
			</div>
			<div id="pSystem_menu-2" style="float: left; margin: 0 129px 0 151px;">
				<div id="pSystem_menu_text-2" class="pSystem-text-1">소독멸균 시스템</div>
				<div id="pSystem_menu_selected-2" class="pSystem_selected" style="width: 165px; height: 4px;"></div>
			</div>
			<div id="pSystem_menu-3" style="float: left;">
				<div id="pSystem_menu_text-3" class="pSystem-text-1">책임보증 프로그램</div>
				<div id="pSystem_menu_selected-3" class="pSystem_selected" style="width: 188px; height: 4px;"></div>
			</div>
			<div style="width: 800px; height: 3px; background: transparent linear-gradient(90deg, #C8500F 0%, #FCA876 100%) 0% 0% no-repeat padding-box; opacity: 1; float: left;"></div>
		</div>
		<div id="pSystem_Detail-1" class="detail_page">
			<div class="pSystem-text-2">"아프지 않은 마인드치과"</div>
			<img class="pSystem-img-1" src="../img/1-4/01/graph01.png">
			<div class="pSystem-text-3">마인드치과에서 진료를 받으신 환자분들 <span class="pSystem-bold">97%</span>가
			아프지 않았다고 대답하셨습니다.</div>
			<div class="margin-bar margin-bar4"></div>
			<img class="pSystem-img-2" src="../img/1-4/01/graphic01.png">
			<div class="pSystem-text-4">치과치료는 아프다?</div>
			<div class="pSystem-text-3" style="line-height: 44px;">마인드치과에서는 <span class="pSystem-bold">“무통마취 시스템”</span>을 
			통해 환자분들의 통증을 최소화 합니다.</div>
			<div style="width: 800px; height: 380px; margin: 79px auto 0;">
				<div class="pSystem-border">
					<div class="pSystem-text-5">첫째,</div>
					<div class="pSystem-text-6">카트리지 워머를 이용해
					마취약을 체온과
					비슷한 37도로 유지</div>
					<img class="pSystem-img-3" src="../img/1-4/01/1401.png">
				</div>
				<div class="pSystem-border" style="margin: 0 10px;">
					<div class="pSystem-text-5">둘째,</div>
					<div class="pSystem-text-6">마취 크림을 통해
					최소한의
					통증까지 제거</div>
					<img class="pSystem-img-3" src="../img/1-4/01/1402.png">
				</div>
				<div class="pSystem-border">
					<div class="pSystem-text-5">셋째,</div>
					<div class="pSystem-text-6">일반 주사바늘 ½ 두께의 얇은
					주사바늘을 이용해 마취제를
					아주 느린 속도로 주입</div>
					<img class="pSystem-img-3" src="../img/1-4/01/1403.png">
				</div>
			</div>
		</div>
		<div id="pSystem_Detail-2" class="detail_page">
			<div class="pSystem-text-77">'대학병원과 동일한 교과서적 소독 멸균 시스템'</div>
			<img class="pSystem-img-4" src="../img/1-4/02/1404.png">
			<div class="margin-bar margin-bar5"></div>
			<div class="pSystem-text-7">사용된 기구는 수거 즉시
			FDA의 가이드라인에 따라 소독, 멸균됩니다.</div>
			<div style="width: 800px; height: 600px; margin: 80px auto 100px;">
				<img class="pSystem-img-5" src="../img/1-4/02/1405.png">
				<img class="pSystem-img-5" src="../img/1-4/02/1406.png">
				<img class="pSystem-img-5" src="../img/1-4/02/1407.png">
				<img class="pSystem-img-5" src="../img/1-4/02/1408.png">
			</div>
			<img class="pSystem-img-6" src="../img/1-4/02/graph02.png">
			<div class="pSystem-text-7" style="margin-top: 99px;">마인드 치과에서는 환자분의 구강 내외는 물론,
			진료 환경까지 청결한 상태로 유지합니다.</div>
		</div>
		<div id="pSystem_Detail-3" class="detail_page">
			<div style="text-align: center; white-space: pre-line;"><span class="pSystem_text-8">MIND WARRANTY</span>
			<span class="pSystem_text-9">“마인드 치과만의 책임보증 프로그램”</span>
			</div>
			<img class="pSystem-img-7" src="../img/1-4/03/warranty01.png">
			<div class="margin-bar margin-bar5"></div>
			<div class="pSystem-text-10">치료 중뿐만 아니라 치료 후에도, 사후관리까지 책임지겠습니다.</div>
			<div class="pSystem-text-11">정확한 진단과 정직한 마인드로 내원하시는 모든 환자분들께 책임 있는 진료를 제공합니다.
			한 번 치료한 치아는 끝까지 책임지며,
			마인드 치과에서 오랫동안 마음 편히 관리 받으실 수 있도록 노력하겠습니다.</div>
			<img class="pSystem-img-8" src="../img/1-4/03/graph03.png">
			<div class="pSystem-text-12">마인드치과에서는 정확한 진단과 최상의 결과를 위해 안전하게 검증된 의료기기와 재료만을 사용합니다.
			언제든지 안심하고 치료를 받으실 수 있도록 사후관리까지 철저히 책임지는 “책임보증 프로그램”을 운영합니다.</div>
			<div class="margin-bar margin-bar5"></div>
			<div class="pSystem-text-10">“오스템 정품 인증서 제공”</div>
			<div class="pSystem-text-11">마인드치과에서는 최고급형 오스템BA 임플란트만을 사용합니다.
			수술 후 모든 분들께 임플란트 정품 인증서를 제공해드립니다.</div>
			<img class="pSystem-img-9" src="../img/1-4/03/warranty02.png">
		</div>
	</div>
	
	
	<div class="top-bar top-bar4 topBarM">
		<div class="pSysd1">
			<div id="pSystem_menu-1m" style="float: left;">
				<div id="pSystem_menu_text-1m" class="pSystem-text-1" style="color: #C8500F; font-weight: 800;">무통마취 시스템</div>
				<div id="pSystem_menu_selected-1m" class="pSystem_selected" style="width: 90px; height: 4px; background-color: #B4AAA0;"></div>
			</div>
			<div id="pSystem_menu-2m" style="float: left; margin: 0 16px 0 24px;">
				<div id="pSystem_menu_text-2m" class="pSystem-text-1">소독멸균 시스템</div>
				<div id="pSystem_menu_selected-2m" class="pSystem_selected" style="width: 90px; height: 4px;"></div>
			</div>
			<div id="pSystem_menu-3m" style="float: left;">
				<div id="pSystem_menu_text-3m" class="pSystem-text-1">책임보증 프로그램</div>
				<div id="pSystem_menu_selected-3m" class="pSystem_selected" style="width: 100px; height: 4px;"></div>
			</div>
			<div style="width: 320px; height: 3px; background: transparent linear-gradient(90deg, #C8500F 0%, #FCA876 100%) 0% 0% no-repeat padding-box; opacity: 1; float: left;"></div>
		</div>
		
		<div id="pSystem_Detail-1m" class="detail_page">
			<div class="pSystem-text-2">"아프지 않은 마인드치과"</div>
			<img class="pSystem-img-1" src="../img/1-4/01/graph01.png">
			<div class="pSystem-text-3">마인드치과에서 진료를 받으신
			환자분들 <span class="pSystem-bold">97%</span>가
			아프지 않았다고 대답하셨습니다.</div>
			<img class="pSystem-img-2" src="../img/1-4/01/graphic01.png">
			<div class="pSystem-text-4">치과치료는 아프다?</div>
			<div class="pSystem-text-3" style="margin:68px auto 48px;">마인드치과에서는
			<span class="pSystem-bold">“무통마취 시스템”</span>을 통해
			환자분들의 통증을 최소화 합니다.</div>
			<div style="width: 320px; height: 880px; margin-left: 40px;">
				<div class="pSystem-border">
					<div class="pSystem-text-5">첫째,</div>
					<div class="pSystem-text-6">카트리지 워머를 이용해 마취약을
					체온과 비슷한 37도로 유지</div>
					<img class="pSystem-img-3" src="../img/1-4/01/1401.png">
				</div>
				<div class="pSystem-border">
					<div class="pSystem-text-5">둘째,</div>
					<div class="pSystem-text-6">마취 크림을 통해 최소한의
					통증까지 제거</div>
					<img class="pSystem-img-3" src="../img/1-4/01/1402.png">
				</div>
				<div class="pSystem-border">
					<div class="pSystem-text-5">셋째,</div>
					<div class="pSystem-text-6">일반 주사바늘 ½ 두께의 얇은 바늘을
					이용해 마취제를 아주 느린 속도로 주입</div>
					<img class="pSystem-img-3" src="../img/1-4/01/1403.png">
				</div>
			</div>
		</div>
		
		<div id="pSystem_Detail-2m" class="detail_page">
			<div class="pSystem-text-77">대학병원과 동일한
			교과서적 소독 멸균 시스템</div>
			<img class="pSystem-img-4" src="../img/1-4/02/1404.png">
			<div class="margin-bar margin-bar5"></div>
			<div class="pSystem-text-7">사용된 기구는 수거 즉시 FDA
			가이드라인에 따라 소독, 멸균됩니다.</div>
			<div style="width: 320px; height: 240px; margin: 48px auto 49.15px;">
				<img class="pSystem-img-5" src="../img/1-4/02/1405.png">
				<img class="pSystem-img-5" src="../img/1-4/02/1406.png">
				<img class="pSystem-img-5" src="../img/1-4/02/1407.png">
				<img class="pSystem-img-5" src="../img/1-4/02/1408.png">
			</div>
			<img class="pSystem-img-6" src="../img_m/1/graph02.png">
			<div class="pSystem-text-78">마인드 치과에서는 환자분의 구강 내외는 물론,
			진료 환경까지 청결한 상태로 유지합니다.</div>
		</div>
		
		<div id="pSystem_Detail-3m" class="detail_page">
			<div style="width: 340px; text-align: center; white-space: pre-line;"><span class="pSystem_text-8">MIND WARRANTY</span>
			<span class="pSystem_text-9">“마인드 치과만의 책임보증 프로그램”</span>
			</div>
			<img class="pSystem-img-7" src="../img/1-4/03/warranty01.png">
			<div class="margin-bar margin-bar5"></div>
			<div class="pSystem-text-10">치료 중뿐만 아니라 치료 후에도, 사후관리까지
			책임지겠습니다.</div>
			<div class="pSystem-text-11">정확한 진단과 정직한 마인드로 내원하시는 모든 환자분들께
			책임 있는 진료를 제공합니다. 한 번 치료한 치아는
			끝까지 책임지며, 마인드 치과에서 오랫동안 마음 편히
			관리 받으실 수 있도록 노력하겠습니다.</div>
			<img class="pSystem-img-8" src="../img/1-4/03/graph03.png">
			<div class="pSystem-text-12">마인드치과에서는 정확한 진단과 최상의 결과를
			위해 안전하게 검증된 의료기기와 재료만을 사용합니다.
			언제든지 안심하고 치료를 받으실 수 있도록 사후관리까지
			철저히 책임지는 “책임보증 프로그램”을 운영합니다.</div>
			<div class="margin-bar margin-bar5"></div>
			<div class="pSystem-text-10">“오스템 정품 인증서 제공”</div>
			<div class="pSystem-text-11">마인드치과에서는 최고급형 오스템BA 임플란트만을
			사용합니다. 수술 후 모든 분들께
			임플란트 정품 인증서를 제공해드립니다.</div>
			<img class="pSystem-img-9" src="../img/1-4/03/warranty02m.png">
			<img class="pSystem-img-9" src="../img/1-4/03/warranty03m.png">
		</div>
	</div>
</div>
<script type="text/javascript" src="../js/inform.js"></script>

<script>
$(document).ready(function(){
	$('#menu_bar-1').css("font-weight", "800");
	$('#menu_bar-1').css("color", "#C8500F");
	$('#menu_selected_bar-1').css("background-color", "#B4AAA0");
	$('#menu_dropDown').css('display', 'block');
	$('#menu_dropDown-1').css("display", "block");
	$('#mdt-04').css("font-weight", "800");
	
	$('#header_wrap').mouseleave(function(){//헤더 벗어나도 유지
		$('#menu_bar-1').css("font-weight", "800");
		$('#menu_bar-1').css("color", "#C8500F");
		$('#menu_selected_bar-1').css("background-color", "#B4AAA0");
		$('#menu_dropDown').slideDown(200);
		$('#menu_dropDown-1').css("display", "block");
		$('#mdt-04').css("font-weight", "800");		
	});
	if('${principle}' == 2){
		$('.pSystem-text-1').css("color", "#9B948C").css("font-weight", "bold");
		$('.pSystem_selected').css("background-color", "#FFFFFF");
		$('.detail_page').css("display", "none");
		$('#pSystem_menu_text-2').css("color", "#C8500F").css("font-weight","800");
		$('#pSystem_menu_selected-2').css("background-color", "#B4AAA0");
		$('#pSystem_Detail-2').show();
		$('#pSystem_menu_text-2m').css("color", "#C8500F").css("font-weight","800");
		$('#pSystem_menu_selected-2m').css("background-color", "#B4AAA0");
		$('#pSystem_Detail-2m').show();
	}else if('${principle}' == 3){
		$('.pSystem-text-1').css("color", "#9B948C").css("font-weight", "bold");
		$('.pSystem_selected').css("background-color", "#FFFFFF");
		$('.detail_page').css("display", "none");
		$('#pSystem_menu_text-3').css("color", "#C8500F").css("font-weight","800");
		$('#pSystem_menu_selected-3').css("background-color", "#B4AAA0");
		$('#pSystem_Detail-3').show();
		$('#pSystem_menu_text-3m').css("color", "#C8500F").css("font-weight","800");
		$('#pSystem_menu_selected-3m').css("background-color", "#B4AAA0");
		$('#pSystem_Detail-3m').show();
	}
});
</script>
