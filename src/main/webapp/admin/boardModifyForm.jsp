...
<div id="ab_div">
	<div id="ab_title"><span style="display: table-cell; vertical-align: middle;">게시글 수정</span></div>
	<form id="abmf_frm" method="post"  enctype="multipart/form-data">
		<table id="abwf-tbl">
			<tr>
				<th>제목</th>
				<td><input type="text" id="abSubject" name="abSubject" value="${abDTO.abSubject }"></td>
			</tr>
			<tr>
				<th>글 종류</th>
				<td>
					<select id="abType" name="abType" style="width: 150px; font-size: 15px; height: 30px;">
			...
			<tr>
				...
				<td><textarea name="abContent" id="abContent" rows="10" cols="100" style="width: 100%;">${abDTO.abContent }</textarea></td>
			</tr>
			<tr>
				<th>썸네일 이미지</th>
				<td>
					<input type="file" id="abFileU" name="abFileU" style="width: 300px; height: 25px;">
					<span style="margin-left: 10px; font-size: 14px; color:#9B948C;">※기존 이미지 : ${abDTO.abFile}</span>
				</td>
			</tr>
		</table>
		<input type="hidden" id="abNum" name="abNum" value="${abDTO.abNum }">
	</form>
	<div id="ab_btn_div">
		<a id="ab_modify" class="ab_btn" style="margin-right: 15px;"><span>수정</span></a>
...
$(document).ready(function(){
	$('#abType option:eq(${abDTO.abType})').attr("selected", "selected");
	
	
	var contextPath = "${pageContext.request.contextPath}"
	var abNum = $('#abNum').val();
	
	$('#ab_modify').click(function(){
		if($('#abSubject').val()==''){
			alert('제목을 입력해주세요');
		}
		else {
			oEditors.getById["abContent"].exec("UPDATE_CONTENTS_FIELD", []);
			var formData = new FormData($("#abmf_frm")[0]);
			//alert($('#abSubject').val());
			//alert($('#abContent').val());
			//alert($('#abType').val());
			//alert($('#abFile').val());
