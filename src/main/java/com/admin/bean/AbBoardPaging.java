	...
	public void makePagingHTML() {
		...
		for(int i=startPage; i<=endPage; i++) {
			if(i==currentPage)
				...
				pagingHTML.append("<div class='paging-box'> <span id='paging' onclick='boardPaging("+i+")'>"+i+"</span></div>");
		}
		
		if(endPage<totalP)
			pagingHTML.append("<div class='paging-box'> <span id='paging' onclick='boardPaging("+(endPage+1)+")'>></span></div>");
